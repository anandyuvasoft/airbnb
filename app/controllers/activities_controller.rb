class ActivitiesController < ApplicationController
  ...
  def index
		@activities = PublicActivity::Activity.order("created_at desc").where(recipient_id: current_user.id).paginate(:page => params[:page], :per_page => 5)
    @activities.update_all(is_read: true)
  end


  # TODO improve the code of sending ajax
  def mark_as_read
